...
public class DetailInfoUI : MonoBehaviour
{
    ...
    private void Awake()
    {
        canvasGroup = GetComponent<CanvasGroup>(); // 컴포넌트 차기
    ...
    public void Open(ItemData itemData)
    {
        if (!IsPause && itemData != null)
        {
            ...
            canvasGroup.alpha = 0.00001f; // MovePosition이 alpha가 0보다 클 때만 실행되니 미리 조금만 올리기
    ...
    public void MovePosition(Vector2 screenPos)
    {
        ...
        if (canvasGroup.alpha > 0.0f) // 보이는 상황인지 확인
        {
            ...
            screenPos.x -= Mathf.Max(0, over); // over를 양수로만 사용 (음수일 때는 별도 처리 필요 없음)
