...
public class Enemy : RecycleObject, IBattler, IHealth
{
    ...
    void Awake()
    {
        ...
        attackArea.onPlayerOut += (target) =>
        {
            if (attackTarget == target)         // 공격 대상이 나갔으면
            {
                ...
                {
                    State = EnemyState.Chase;   // 추적 상태 되돌리기
                }
            }
        };
    }
    ...
    protected override void OnEnable()
    {
        ...
        rigid.drag = Mathf.Infinity;    // 무한대로 되어있던 마찰력을 낮춰서 떨어질 수 있게 하기
        HP = maxHP;                     // HP 최대로
    }
    ...
    protected override void OnDisable()
    {
        ...
        agent.enabled = true;                   // agent가 활성화 되어있으면 항상 네브메시 위에 있음
    ...
    IEnumerator DeadSquence()
    {
        ...
        yield return new WaitForSeconds(0.5f);  // 아이템이 바로 떨어지면 어색하므로 약간 대기
        ...
        // 바닥으로 가라앉기 시작
        agent.enabled = false;                  // agent가 활성화 되어있으면 항상 네브메시 위에 있음
        rigid.isKinematic = false;              // 키네마틱을 꺼서 물리가 적용되게 만들기
        rigid.drag = 10.0f;                     // 무한대로 되어있던 마찰력을 낮춰서 떨어질 수 있게 하기

        // 충분히 바닥 아래로 내려갈 때까지 대기
        ...
        // Destroy(this.gameObject);               // 슬라임 삭제
    }
