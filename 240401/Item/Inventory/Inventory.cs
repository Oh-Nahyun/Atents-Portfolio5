...
public class Inventory
{
    ...
    /// 인벤토리 생성자
    /// </summary>
    /// <param name="owner">인벤토리의 송유자</param>
    ...
    public bool AddItem(ItemCode code) ///// 다 찾아봐서 넣을 수 있으면 넣기
    {
    ...
    /// <summary>
    /// 인벤토리의 특정 슬롯에 특정한 아이템을 1개 추가하는 함수
    /// </summary>
    /// <param name="code">추가할 아이템의 코드</param>
    /// <param name="slotIndex">아이템을 추가할 슬롯의 인덱스</param>
    /// <returns>true면 추가 성공, false면 추가 실패</returns>
    public bool AddItem(ItemCode code, uint slotIndex) ///// 특정 칸에 넣을 수 있으면 넣기
    {
        bool result = false;
        ...
        if (IsValidIndex(slotIndex))                // 인덱스가 적절한지 확인
        {
            ...
            if (slot.IsEmpty)
            {
                // 슬롯이 비었으면 ///// 인벤토리가 비어있는 경우
                slot.AssignSlotItem(data);          // 그대로 아이템 설정
                result = true;
            }
            else
            {
                // 슬롯이 안비어있다. ///// 인벤토리가 비어있지 않는 경우
                if (slot.ItemData == data)
                {
                    // 같은 종류의 아이템이면 ///// 종류가 같은 경우
                    result = slot.IncreaseSlotItem(out _);   // 아이템 증가 시도 ///// 아이템의 개수를 올린다.
                }
                else
                {
                    // 다른 종류의 아이템이면 ///// 다른 종류일 경우
    ...
    public void MoveItem(uint from, uint to) ////////// 함수 프로토타입 (함수의 제일 윗줄) - 함수 바디 (함수 안의 코드)
    {
        // from 지점과 to 지점은 서로 다른 위치이고, 모두 valid한 인덱스이어야 한다.
        if ((from != to) && IsValidIndex(from) && IsValidIndex(to))
        {
            ...
            if (!fromSlot.IsEmpty)
            {
                ...
                {
                    toSlot = slots[to];                     // temp 슬롯이 아니면 슬롯만 저장
                }

                if (fromSlot.ItemData == toSlot.ItemData)
                {
                    // 같은 종류의 아이템 => to에 채울 수 있는데까지 채운다. to에 넘어간 만큼 from을 감소시킨다.
                    toSlot.IncreaseSlotItem(out uint overCount, fromSlot.ItemCount); // from이 가진 개수만큼 to에 추가

                    // 5/10 -> 7/10으로 옮긴 상황이라고 가정하면, over == 2, (5 - 2)만큼 감소
                    fromSlot.DecreaseSlotItem(fromSlot.ItemCount - overCount); // from에서 to로 넘어간 개수만큼만 감소

                    //Debug.Log($"[{from}]번 슬롯에서 [{to}]번 슬롯으로 아이템 합치기 시도");
                }
                else
                {
                    ...
                    // 드래그가 끝났을 때, 임시 슬롯에 있던 아이템이 -> to, 원래 to -> 원래 from으로 이동하기
                    if (fromIsTemp)
                    {
                        // temp 슬롯에서 to 슬롯으로 옮기는 경우
                        //      returnSlot에 toSlot 슬롯의 데이터 넣고,
                        //      toSlot에 temp 슬롯의 데이터 넣기

                        // slots[TempSLot.FromIndex]가 될 수 있는 의미 분석
                        // - 비어있다. => 기존 그대로 처리
                        // - 들어있다. >> (1) 다른 종류의 아이템 => temp와 to 스왑만
                        //           >> (2) 같은 종류의 아이템 => 합치기

                        InvenSlot returnSlot = slots[TempSlot.FromIndex]; // temp가 기록해놓은 FromIndex에 있는 슬롯
    ...
    void SwapSLot(InvenSlot slotA, InvenSlot slotB)
    {
        ...
        /*
        bool toIsTemp = (slotB == tempSlot);

        slotA.AssignSlotItem(slotB.ItemData, slotB.ItemCount, slotB.IsEquipped);
        if (slotA.IsEquipped)
        {
            ItemData_Equip equipData = slotA.ItemData as ItemData_Equip;
            Owner[equipData.EquipType] = slotA;
            //    if (from == TempSlot)
            //    {
            ...
            //    else
            //    {
            //        Owner[equipData.EquipType] = from;
            //    }
        }

        slotB.AssignSlotItem(tempData, tempCount, tempEquip);
        if (slotB.IsEquipped && !toIsTemp)
        {
            ItemData_Equip equipData = slotB.ItemData as ItemData_Equip;
            Owner[equipData.EquipType] = slotB;
            //    if (to == TempSlot)
            //    {
            //        Owner[equipData.EquipType] = slots[TempSlot.FromIndex];
            //    }
    ...
    public void DividItem(uint slotIndex, uint count)
    {
        if (IsValidIndex(slotIndex))
        {
            ...
            // count가 슬롯에 들어있는 개수보다 크면 슬롯에 들어있는 개수까지만 사용
            count = System.Math.Min(count, slot.ItemCount);
            //if (slot.ItemCount < count) // 아래 코드와 같은 의미의 코드
    ...
    public void SlotSorting(ItemSortBy sortBy, bool isAcending = true)
    {
        ...
        switch (sortBy)
        {
            case ItemSortBy.Code:
                temp.Sort((current, other) =>       // x, y는 temp 리스트에 들어있는 요소 중 2개
                {
    ...
    {
        /// 실습_240314
        /// 아래 출력 예시와 같이 출력하도록 Test_InventoryPrint() 완성하기
        /// 출력 예시 : [루비(1/10), 사파이어(2/3), (빈칸), 에메랄드(3/5), (빈칸), (빈칸)]
...
/// 생각해보기_240314
/// 인벤토리가 해야할 일(함수)은?
/// [정답] 아이템 추가 / 아이템 제거 / 아이템 이동 / 아이템 나누기 / 아이템 정렬
