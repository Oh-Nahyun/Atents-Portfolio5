...
public class Player : MonoBehaviour, IHealth, IMana, IEquipTarget, IBattler
{
    ...
    /// <returns>null이면 장비되어있지 않음. null이 아니면 해당 슬롯에 있는 아이템이 장비되어있음.</returns>
        ...
        if (equip != null) // 장비 가능한 아이템인지 확인
        {
            ...
            switch (part)
            {
                case EquipType.Weapon:
                    ...
                    onWeaponEffectEnable = weapon.EffectEnable;
                    onWeaponBladeEnable = weapon.BladeVolumeEnable;
    ...
    /// </summary>
    void WeaponBledeDisable()
    {
        onWeaponBladeEnable?.Invoke(false);
    }

    /// <summary>
    /// 아이템이 장비되었을 때 실행되는 함수
    /// </summary>
    ...
    public void Defence(float damage)
    {
        if (IsAlive)
        {
            ...
            WeaponBledeDisable(); // 공격 중에 피격 당할 경우를 대비
    ...
    {
        if (CurrentMoveMode == MoveMode.Walk && MP > 0) // 걷기 모드이고 마나가 있을 때 스킬 사용 가능
        {
