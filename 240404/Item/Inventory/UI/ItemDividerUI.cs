...
public class ItemDividerUI : MonoBehaviour
{
    ...
    // DividCountInputField, DividSlider는 서로 값이 연동된다.
    // 아이템을 나누는 개수는 반드시 1 ~ (슬롯에 들어있는 아이템 수 - 1) 이어야 한다.
    // OK 버튼을 누르면 슬롯에서 설정된 개수만큼 아이템이 빠지고 빠진만큼 Temp 슬롯에 들어간다.
    ...
    /// <summary>
    /// OK 버튼을 눌렀을 때 실행되는 델리게이트 (uint : targetSlot의 인덱스, uint : dividCount)
    /// </summary>
    public Action<uint, uint> onOkClick;

    /// <summary>
    /// Cancel 버튼을 눌렀을 때 실행되는 델리게이트
    /// </summary>
    ...
    /// <summary>
    /// 클릭하면 실행되는 함수 (UI 밖을 클릭했는지를 체크하기 위한 용도)
    /// </summary>
    /// <param name="_"></param>
    private void OnClick(InputAction.CallbackContext _)
    {
        if (!MousePointInRect()) // 마우스 포인터가 UI의 rect 안에 있는지 확인
        {
    ...
    bool MousePointInRect()
    {
        ...
        Vector2 diff = screenPos - (Vector2)transform.position; // 이 UI의 피봇에서 마우스 포인터가 얼마나 떨어져있는지 계산
